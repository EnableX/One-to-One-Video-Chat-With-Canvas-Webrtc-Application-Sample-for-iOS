...
class EnxConfrenceViewController: UIViewController {
    ...
    var activeStream : EnxStream!
    var canvasCheck : Bool! = false
    var listOfParticipantInRoom  = [Any]()
    var streamArray : [EnxStream] = []
    var colorList :[UIColor] = [UIColor.red,UIColor.green,UIColor.blue,UIColor.yellow, UIColor.brown]
    ...
    @IBAction func startStopCanvas(_ sender: UIButton) {
        guard remoteRoom != nil else {
            return
        }
        if(!sender.isSelected){
            remoteRoom.startCanvas(canvasView)
        }
        else{
            remoteRoom.stopCanvas()
        }
    }
    @IBAction func changeColor(_ sender: UIButton){
        
        if(colorCount < colorList.count){
            canvasView.backgroundColor = colorList[colorCount]
            colorCount += 1;
            if(colorCount >= colorList.count){
                colorCount = 0;
            }
...
extension EnxConfrenceViewController : EnxRoomDelegate, EnxStreamDelegate {
    ...
    */
    ...
    func room(_ room: EnxRoom?, didStartCanvasACK Data: [Any]?) {
        //To Do
        startCanvas.isSelected = true;
        canvasView.isHidden = false;
        changesColor.isHidden = false
        playerViewTop.constant = (canvasView.frame.origin.y + canvasView.frame.size.height)
        self.view.setNeedsLayout()
    }
    /*To Ack. User on Stop Canvas*/
    func room(_ room: EnxRoom?, didStoppedCanvasACK Data: [Any]?) {
        //To Do
